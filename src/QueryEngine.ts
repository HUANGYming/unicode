...
export class QueryEngine {
  ...
  async *submitMessage(
    ...
    if (result.type === 'assistant') {
      // Find the last assistant message that has a text block. OpenRouter/Bedrock may stream
      // thinking and redacted_thinking as separate messages after the text message, so we
      // search backwards through all messages for the last one containing a text block.
      const lastMsgWithText = [...messages].reverse().find(
        m => m.type === 'assistant' && Array.isArray(m.message.content) &&
          m.message.content.some(c => c.type === 'text')
      )
      const textBlock = lastMsgWithText?.type === 'assistant'
        ? [...lastMsgWithText.message.content].reverse().find(c => c.type === 'text')
        : undefined
      if (textBlock?.type === 'text' && !SYNTHETIC_MESSAGES.has(textBlock.text)) {
